package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1360:uint = 1;
      
      protected static const const_1359:int = 5;
      
      protected static const const_1001:uint = 7;
      
      protected static const const_1356:uint = 4;
      
      protected static const const_1358:uint = 3;
       
      
      protected var var_874:int;
      
      protected var var_2187:String = "";
      
      protected var var_355:int;
      
      protected var var_472:XML;
      ...
      {
         var_2186 = param1;
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,var_355));
      }
      ...
      protected function prepareLibrary() : void
      {
         ...
         var_472 = new XML(bytes.readUTFBytes(bytes.length));
      }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(var_864)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,var_355));
         }
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               var_355 = HTTPStatusEvent(param1).status;
               debug("Load event STATUS " + var_355 + " for file \"" + var_1.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
               debug("Load event UNLOAD for file \"" + var_1.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,var_355));
               break;
            case ProgressEvent.PROGRESS:
               debug("Load event PROGRESS for file \"" + var_1.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,var_355));
               break;
            case IOErrorEvent.IO_ERROR:
               failure("IO Error, send or load operation failed for file \"" + var_1.contentLoaderInfo.url + "\"");
               if(!handleHttpStatus(var_355))
               {
         ...
         if(var_421 == LibraryLoader.const_1358)
         {
            if(analyzeLibrary())
            {
               var_421 |= 0;
         ...
         if(var_421 == LibraryLoader.const_1001)
         {
            var_1306 = true;
            prepareLibrary();
            removeEventListeners();
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,var_355));
         }
      }
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_874 > 0)
            {
               var_1.close();
               var_1.unload();
               addRequestCounterToUrlRequest(var_310,const_1359 - var_874);
               var_1.load(var_310,_context);
               --var_874;
               return true;
            }
