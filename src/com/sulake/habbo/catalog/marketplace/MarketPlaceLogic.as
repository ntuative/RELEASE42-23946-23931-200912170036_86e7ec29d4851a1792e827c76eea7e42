package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1596:int = 3;
      
      public const const_1414:int = 2;
      ...
      private function onConfirmDialogEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            ...
            case "confirm_buy_button":
               var_70.buyMarketPlaceOffer(var_462);
               if(var_136)
               {
                  var_136.dispose();
               }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            if(parser.offerId > 0)
            {
               newItem = var_224.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
               var_224.remove(parser.requestedOfferId);
               newItem.offerId = parser.offerId;
               newItem.price = parser.newPrice;
               var_224.add(parser.offerId,newItem);
            }
            else
            {
               var_224.remove(parser.requestedOfferId);
         ...
         {
            updateItem = var_224.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               var_224.add(parser.offerId,updateItem);
            }
            var_224.remove(parser.requestedOfferId);
            var_462 = parser.offerId;
            showConfirmation(const_1414);
            _visualization.listUpdatedNotify();
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_650.add(_loc4_.offerId,_loc5_);
         }
      ...
      public function redeemSoldOffers() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            ...
            if(_loc3_.status == MarketPlaceOfferState.const_470)
            {
      ...
      private function showConfirmation(param1:int) : void
      {
         ...
         switch(param1)
         {
            case const_1530:
               ...
               var_136 = _windowManager.buildFromXML(_loc2_) as IWindowContainer;
               break;
            case const_1414:
         ...
         if(var_136)
         {
            var_136.procedure = onConfirmDialogEvent;
            var_136.visible = true;
            var_136.center();
            _loc3_ = var_224.getValue(var_462) as MarketPlaceOfferData;
            if(!_loc3_)
            {
               return;
            }
            _loc4_ = var_136.findChildByName("item_name") as ITextWindow;
            if(_loc4_ != null)
            {
               _loc4_.text = "${roomItem.name." + _loc3_.furniId + "}";
            }
            _loc5_ = var_136.findChildByName("item_desc") as ITextWindow;
            if(_loc5_ != null)
            {
               _loc5_.text = "${roomItem.desc." + _loc3_.furniId + "}";
            }
            _loc6_ = var_136.findChildByName("item_price") as ITextWindow;
            if(_loc6_ != null)
            {
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_224.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
