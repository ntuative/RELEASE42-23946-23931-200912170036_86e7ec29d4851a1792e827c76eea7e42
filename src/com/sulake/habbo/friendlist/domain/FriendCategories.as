package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         findCategory(FriendCategory.const_795).received = true;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc2_.updatedFriends)
         {
            ...
            var_412.messenger.setFollowingAllowed(_loc4_.id,_loc4_.followingAllowed && _loc4_.online);
            _loc6_ = isFriendOnline(_loc4_.id);
            if(_loc6_ && !_loc4_.online)
            {
               var_412.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_412.notifications.addOfflineNotification(_loc4_.name,_loc4_.realName);
            }
            if(!_loc6_ && _loc4_.online)
            {
               var_412.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_412.view.setNewMessageArrived();
               var_412.notifications.addOnlineNotification(_loc4_.name,_loc4_.figure,_loc4_.realName);
            }
      ...
      {
         return var_930[param1];
      }
      
      private function flushReceivedStatus() : void
      {
         var _loc1_:* = null;
         for each(_loc1_ in var_316)
         {
